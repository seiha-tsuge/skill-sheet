...
APIおよびフロントエンドの開発、そして単体テストを実施しました。バックエンド開発では、Flaskを使用して、ユーザー管理機能用のAPIを開発しました。フロントエンド開発では、ゼンリンAPIや内製API(携帯基地局情報や自動車の走行データ)を利用して、これらの情報を地図上に視覚的に表示する機能を組み込みました。
