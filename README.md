...
取引所との仕様調整、海外ベンダーとのやり取り、英語でのドキュメント作成
デザイナーチームによる UI 設計の場に参加。UI 設計のキャッチアップに努める

仕様については企画者が概要をまとめ、詳細を詰めながら設計・実装を行っていただく形となります。
デザイナーの方と協業しながら進められる方を求めております。
...
ペアプログラミングを実験的に採用し、相互の信頼関係を構築するために、日々対話を通じ

バックエンドは、TDD およびクリーンアーキテクチャを採用することで品質管理を徹底。AWS は初めて触れる技術でしたが、日々の開発を通じてスキルを徹底してキャッチアップ  
年始に受注予定案件を前倒しで開発に着手するも、同時に複数案件を受注できたため、一旦プロジェクトを中止

・フロントエンドからバックエンドまで一括した開発経験
・アーキテクチャへの理解・知見があり、自身でアーキテクチャを考え実装した経験
・AWSインフラに関して基本的な知識保有者
・チームマネジメント経験 1年以上
