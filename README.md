...
私たちはAPIおよびフロントエンドの開発を行い、単体テストを実施しました。バックエンドの部分では、Flaskを活用してユーザー管理機能用のAPIを開発。フロントエンドではゼンリンAPIや内製API（携帯基地局情報や自動車の走行データ）を利用して、これらのデータを地図上で視覚的に表示する機能を組み込みました。
