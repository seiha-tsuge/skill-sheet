...
フロントエンド開発の経験を生かして、開発経験が浅いメンバーに積極的にフィードバックすることで、チームの技術力向上に貢献しました。
さらに、 ZIP ファイルのやりとりを GitHubに改めたり、コードレビューの文化を持ち込んだりするなど、業務の効率を上げる工夫を行いました。
そして、ソースコードを積極的にコンポーネント化し、変更に強いソースコードにリファクタリングを行いました。
...
- コードレビューが無い文化にコードレビューを持ち込み、経験が浅いメンバーに積極的にフィードバックすることで、チームの技術力を向上
- コンポーネント化されていないソースコードを積極的にコンポーネント化し、変更に強いソースコードにリファクタリングを行う
- 参画した当初はアンチパターンがよく見受けられる品質の悪いソースコードをリファクタリングする経験は初めてで、自身のスキルも向上
...
特に「顧客との信頼関係づくり」「顧客のビジョン達成」「チームの業務効率改善」のために、以下のような取り組みをしてきました。
...
1. チームメンバーと日々意見を出し合う
1. ビジョン達成のために必要なことを顧客と考え、仕様を策定する
1. 顧客と要件や仕様について協議し、詳細を詰めながら設計・実装を行う
1. 実装方針について前もって議論を重ね、実装後も相互にコードレビューをして、品質向上に尽力する
...
1. チームでの行動指針を定める
　定めた指針に従って行動していたか、毎週振り返ることでチームの成長を促しました。

1. バックエンドチームと意見を出し合いながらAPIを設計
...
- メンバー全員フルリモートでの開発だったが、バーチャルオフィス（Gather.Town） を利用し、対面と遜色ないコミュニケーションをとる
- チームでの行動指針を定め、その指針に従って行動していたかを、毎週振り返ることでチームとして成長
- 顧客と隔週でゲーム会を開催し、顧客との距離感を縮める
- 実装方針について前もって議論を重ね、実装後も相互にコードレビューすることで品質を向上
- 新機能開発ではライブラリの調査から導入まで一人称で対応
- バックエンドチームと意見を出し合いながらAPIを設計
- 顧客が掲げるビジョンを達成するためにどのようなことが必要なのかともに考え、仕様を策定
...
さらに、TDDとクリーンアーキテクチャを採用し、バックエンドの品質管理を徹底しました。
AWSは当時未経験でしたので、日々の開発を通じたキャッチアップに努めました。
...
このプロジェクトに私が最も貢献したのは、リソースに必要な機能を1人称で開発したことです。
また、このプロジェクトを通じて、TypeScriptとFlaskの技術を身につけました。
...
- コードレビューの文化に初めて触れ、ベテランエンジニアから指摘を受けることで、技術力を向上
...
このプロジェクトで、Vue.js, Wijmo, Python, Djangoの技術を会得しました。
さらに、担当画面のバックエンド・フロントエンドを単独で開発し、プロジェクトの一翼を担いました。
API開発とフロントエンド開発を同時にこなすことで、幅広い視野を獲得できたと実感しています。
...
- 自信が担当した画面のバックエンド、フロントエンドともに 1 人称で開発しプロジェクトに貢献
...
- 作成した設計書をチームレビュー、リーダーレビュー、元請けレビュー、顧客レビューとレビューをたくさん指摘を受け、考慮が足りない点を知ることで成長
...
週２の個人開発をデザインから行うことでUI設計の実践や、最新技術を採用することでモダンな開発手法を身につけるs
...
私はテクニカル的には、Pythonを利用したWebアプリケーション開発を得意としております。さらにバックエンド開発だけではなく、フロントエンドの開発も可能で御座います。過去案件においては、Web関連の能力・知識を周囲から高い評価を頂き、ウォーターフォール開発では上流工程のリーダー・サブリーダーも任せていただくこともありました。そこでもただ任された業務をそのままこなすのではなく、チームにおける円滑な開発プロセスを意識し、特にレビューをする際にも、ただのレビューで終わらすのではなく相手への伝え方や、ネクストアクションや積み残しタスクの視える化や、抜け漏れのない業務の徹底を行い、生産性向上とモチベーションアップを意識して行動に落とし込んでまいりました。また、私が仕事に取り組む中で最も大切にしていることは、チームでのノウハウ共有です。ドキュメント管理を徹底し、チームメンバーには積極的に共有、そして全員がそのノウハウを共有する文化を醸成することで、「1人の学びを皆の学びにする」筋肉質かつ骨太なチームの構築経験が御座います。今後のプロジェクトにおいても同様に、より良い環境を作り上げ、常にPDCAを回し続けながら、さらに自分自身を成長させていきたい決意です。
何卒宜しくお願いします。
